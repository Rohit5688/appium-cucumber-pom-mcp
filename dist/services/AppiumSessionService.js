...
export class AppiumSessionService {
     ...
     */
    async startSession(projectRoot, profileName, workflowSteps) {
        if (this.driver) {
     ...
     */
    async performAction(action, selector, value, captureAfter = true, verboseCapture = false) {
        this.ensureSession();
        const d = this.driver;
        try {
            switch (action) {
                case 'tap': {
                    if (!selector)
                        throw new Error('"tap" requires a selector argument.');
                ...
                case 'type': {
                    if (!selector)
                        throw new Error('"type" requires a selector argument.');
                    const el = await d.$(selector);
                    await el.waitForDisplayed({ timeout: 10000 });
                ...
                case 'clear': {
                    if (!selector)
                        throw new Error('"clear" requires a selector argument.');
                    const el = await d.$(selector);
                    await el.clearValue();
                    break;
                }
    ...
    extractTopElements(xml) {
        ...
        while ((match = pattern.exec(xml)) !== null && elements.length < 8) {
            ...
            if (skip.some(s => rawType.includes(s)))
                continue;
