...
export class ProjectSetupService {
    ...
    async setup(projectRoot, platform = 'android', appName = 'MyMobileApp') {
        ...
        const requiredUnfilled = unfilledFields.filter(f => ['defaultPlatform', 'platformName', 'automationName', 'deviceName', 'appium:app'].includes(f));
        if (requiredUnfilled.length > 0) {
            ...
            if (typeof (config?.mobile?.defaultPlatform) === 'string' && config.mobile.defaultPlatform.startsWith('CONFIGURE_ME')) {
        ...
        const effectivePlatform = config?.mobile?.defaultPlatform || platform;
    ...
    scanConfigureMe(projectRoot) {
        const configPath = path.join(projectRoot, 'mcp-config.json');
        if (!fs.existsSync(configPath))
            return ['mcp-config.json not found'];
    ...
    scaffoldCucumberConfig(projectRoot, paths) {
...
`;
    ...
    scaffoldBasePage(projectRoot, paths) {
...
`;
    ...
    scaffoldMobileGestures(projectRoot, paths) {
...
`;
    ...
    scaffoldMockServer(projectRoot, paths) {
...
`;
    ...
    scaffoldLocatorUtils(projectRoot, paths) {
    ...
    scaffoldHooks(projectRoot, screenshotOn = 'failure', reporting, paths) {
...
`;
    ...
    scaffoldSampleFeature(projectRoot, paths) {
        const content = `@smoke
...
`;
    ...
    scaffoldActionUtils(projectRoot, elementWait = 10000, paths) {
...
`;
    ...
    scaffoldGestureUtils(projectRoot, paths) {
        const content = `import { browser, $ } from '@wdio/globals';
...
`;
    ...
    scaffoldWaitUtils(projectRoot, elementWait = 10000, paths) {
        const content = `import { browser, $ } from '@wdio/globals';
...
`;
    ...
    scaffoldAssertionUtils(projectRoot, paths) {
        const content = `import { browser, $ } from '@wdio/globals';
...
`;
    ...
    scaffoldTestContext(projectRoot, paths) {
...
`;
    ...
    scaffoldDataUtils(projectRoot, paths) {
...
`;
     ...
     */
    async previewSetup(projectRoot, platform = 'android', appName = 'MyMobileApp') {
        const configPath = path.join(projectRoot, 'mcp-config.json');
        if (!fs.existsSync(projectRoot)) {
            return JSON.stringify({
                preview: true,
     ...
     */
    async upgrade(projectRoot, preview = false) {
     ...
     */
    async previewUpgrade(projectRoot) {
        const configPath = path.join(projectRoot, 'mcp-config.json');
        if (!fs.existsSync(configPath)) {
            return {
     ...
     */
    async repair(projectRoot, platform = 'android') {
        try {
     ...
     */
    async upgradeFromConfig(projectRoot, preview = false) {
        if (preview) {
        ...
        if (codegen.customWrapperPackage) {
            ...
            if (fs.existsSync(basePagePath)) {
                pending.push(`BasePage.ts exists but customWrapperPackage="${codegen.customWrapperPackage}" is set. ` +
        ...
        if (repairResult.repairedFiles && repairResult.repairedFiles.length > 0) {
            applied.push(...repairResult.repairedFiles.map((f) => `Repaired missing file: ${f}`));
        }
        // ─── Summary ─────────────────────────────────────────────────────────────────
        return JSON.stringify({
