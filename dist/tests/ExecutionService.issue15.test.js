...
describe('ExecutionService - Issue #15: Valid Locator Strategies', () => {
    it('should generate snapshot containing accessibility-id selector with ~ prefix', async () => {
        ...
        assert.ok(result.snapshot.includes('~Submit Button'), 'Snapshot should include accessibility-id selector');
        // Should NOT contain invalid *[text()="..."] format
        assert.ok(!result.snapshot.includes('*[text()='), 'Snapshot should NOT contain invalid *[text()= format');
    });
    it('should generate snapshot containing resource-id selector with id= prefix', async () => {
        ...
        const result = await executionService.inspectHierarchy('<test>', xml, '');
        assert.ok(result.snapshot.includes('id=com.app:id/submitBtn'), 'Snapshot should include resource-id selector');
        // Should NOT contain invalid selector format
        assert.ok(!result.snapshot.includes('*['), 'Snapshot should NOT contain selectors starting with *[');
    });
    it('should generate snapshot containing valid XPath selector for text attribute', async () => {
        ...
        assert.ok(result.snapshot.includes('//*[@text="Login"]'), 'Snapshot should include valid XPath with @text attribute');
        // Should NOT generate invalid *[text()="Login"]
        assert.ok(!result.snapshot.includes('*[text()="Login"]'), 'Snapshot should NOT include invalid *[text()= format');
    });
    it('should escape double quotes in text for valid XPath', async () => {
        ...
        assert.ok(result.snapshot.includes('//*[@text="He said &quot;Hello&quot;"]'), 'Snapshot should escape double quotes in XPath');
    });
    ...
    `;
        ...
        assert.ok(accessIdx < resIdx || resIdx === -1, 'Accessibility ID should appear before resource-id in snapshot');
    });
    it('should not generate selectors for invalid text (too long)', async () => {
        ...
        assert.ok(result.snapshot.includes('id=com.app:id/longText'), 'Snapshot should include resource-id');
        // Long text XPath should NOT be present
        assert.ok(!result.snapshot.includes(`@text="${longText}"`), 'Snapshot should NOT include text-based XPath for long text');
    });
    it('should handle iOS elements with name attribute', async () => {
        ...
        assert.ok(result.snapshot.includes('~loginButton'), 'Snapshot should use name attribute for accessibility-id on iOS');
    });
    it('should generate class-based XPath as last resort', async () => {
        ...
        assert.ok(result.snapshot.includes('//android.widget.Button'), 'Snapshot should use class-based XPath as last resort');
    });
    it('regression test: should never generate invalid *[attribute] selectors', async () => {
        ...
        assert.ok(!result.snapshot.includes('*[text()='), 'CRITICAL: Snapshot should NOT contain invalid *[text()= selectors');
        // Should contain VALID XPath instead
        assert.ok(result.snapshot.includes('//*[@text="Submit"]'), 'Snapshot should contain valid XPath //*[@text="Submit"] instead');
    });
