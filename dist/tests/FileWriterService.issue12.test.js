...
describe('FileWriterService.validateAndWrite - Issue #12 Fix (Staging & Rollback)', () => {
    ...
    test.before(() => {
        ...
        fs.writeFileSync(path.join(testProjectRoot, 'tsconfig.json'), JSON.stringify(tsconfig, null, 2), 'utf8');
    });
    // Cleanup
    test.after(() => {
        try {
...
export class LoginPage {
  async login(username: string, password: string) {
    ...
    await this.nonExistentMethod(username);
        ...
        assert.ok(!fs.existsSync(filePath), 'Invalid TypeScript file should NOT be written to disk when validation fails');
    });
...
const x: number = "this is a string"; // Type mismatch
        ...
        assert.ok(!fs.existsSync(stagingDir), 'Staging directory should be cleaned up after failed validation');
    });
  ...
  async login(username: string, password: string): Promise<void> {
    ...
    if (userInput && passInput && submitBtn) {
      ...
      await submitBtn.click();
        ...
        // Verify staging directory was cleaned up
        const stagingDir = path.join(testProjectRoot, '.mcp-staging');
        assert.ok(!fs.existsSync(stagingDir), 'Staging directory should be cleaned up after successful write');
    });
...
export class DryRunPage {
  public async test(): Promise<void> {
    console.log('ok');
        ...
        assert.ok(!fs.existsSync(filePath), 'Dry-run should NOT write files to disk');
    });
...
export class ErrorPage {
  private unknownType: NonExistentType; // This type does not exist
    ...
    test('[ISSUE #12] should not leave .mcp-staging directory on disk after failed validation', async () => {
        ...
        const stagingDir = path.join(testProjectRoot, '.mcp-staging');
        assert.ok(!fs.existsSync(stagingDir), 'Staging directory should be completely cleaned up, even on validation failure');
    });
...
export class CleanupTest {
  public constructor() {
    // cleanup test
        ...
        const stagingDir = path.join(testProjectRoot, '.mcp-staging');
        assert.ok(!fs.existsSync(stagingDir), 'Staging directory should be completely cleaned up after successful write');
    });
...
Given('the user is logged in', async function() {
  ...
  unknownFunction();
        ...
        assert.ok(!fs.existsSync(path.join(testProjectRoot, 'src/features/test.feature')), 'Feature file should not be written if TypeScript validation fails');
    });
...
export class BackupTest {
  public constructor() {
    // backup test
        ...
        const parsed = JSON.parse(result);
        assert.strictEqual(parsed.success, true, 'Write should succeed');
        assert.ok(parsed.filesWritten && Array.isArray(parsed.filesWritten), 'Response should list written files');
...
export class MessageTest {
  public constructor() {
    // message test
        ...
        assert.ok(successParsed.message.includes('successfully') && successParsed.success === true, 'Successful write should explicitly say "successfully"');
    });
});
