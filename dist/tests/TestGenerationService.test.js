...
describe('TestGenerationService — Issue #11 Fix Validation', () => {
    ...
    test('should generate a valid prompt string', async () => {
        const prompt = await service.generateAppiumPrompt('/test/project', 'User logs in with valid credentials', mockConfig, mockAnalysis);
        assert.strictEqual(typeof prompt, 'string');
        assert.ok(prompt.length > 0);
    });
    test('[ISSUE #11] prompt header should identify framework as Appium + WebdriverIO + @wdio/cucumber-framework', async () => {
        const prompt = await service.generateAppiumPrompt('/test/project', 'User logs in', mockConfig, mockAnalysis);
        assert.ok(prompt.includes('Appium + WebdriverIO + @wdio/cucumber-framework'), 'Expected prompt to contain "Appium + WebdriverIO + @wdio/cucumber-framework"');
    });
    test('[ISSUE #11] prompt must NOT contain the word "Playwright"', async () => {
        const prompt = await service.generateAppiumPrompt('/test/project', 'User logs in with valid credentials', mockConfig, mockAnalysis);
        assert.ok(!prompt.includes('Playwright'), 'FAILURE: Prompt contains "Playwright" — this violates Issue #11 fix. AppForge is Appium/WebdriverIO, not Playwright.');
    });
    test('[ISSUE #11] prompt must include explicit constraint against Playwright imports', async () => {
        const prompt = await service.generateAppiumPrompt('/test/project', 'User logs in with valid credentials', mockConfig, mockAnalysis);
        assert.ok(prompt.includes('CRITICAL CONSTRAINT'), 'Expected critical constraint warning');
        assert.ok(prompt.includes('ONLY WebdriverIO'), 'Expected emphasis on WebdriverIO-only usage');
        assert.ok(prompt.includes('driver.$()'), 'Expected mention of WebdriverIO selector syntax');
        assert.ok(prompt.includes('Appium'), 'Expected mention of Appium locator strategies');
        assert.ok(prompt.includes('Do NOT import web testing libraries'), 'Expected warning against other testing frameworks');
    });
    ...
    test('[ISSUE #11] prompt must include critical constraint warning with emphasis marker', async () => {
        const prompt = await service.generateAppiumPrompt('/test/project', 'User logs in with valid credentials', mockConfig, mockAnalysis);
        assert.ok(prompt.includes('⚠️ **CRITICAL CONSTRAINT**'), 'Expected critical constraint warning with ⚠️ emoji');
    });
    test('should handle optional parameters (testName, screenXml, screenshotBase64)', async () => {
        const prompt = await service.generateAppiumPrompt('/test/project', 'User logs in with valid credentials', mockConfig, mockAnalysis, 'LoginTest', undefined, '<xml>test</xml>', 'base64data');
    ...
    test('should include existing code reuse sections', async () => {
        ...
        const prompt = await service.generateAppiumPrompt('/test/project', 'Test a feature', mockConfig, analysisWithCode);
    ...
    test('should include learning prompt if provided', async () => {
        ...
        const prompt = await service.generateAppiumPrompt('/test/project', 'User logs in', mockConfig, mockAnalysis, undefined, learningPrompt);
    ...
    test('[HYBRID] prompt should include CoT reasoning scaffold', async () => {
        ...
        assert.ok(prompt.includes('MANDATORY REASONING PROTOCOL'), `Expected CoT scaffold header in prompt. Got first 500 chars: ${prompt.substring(0, 500)}`);
        // Step 1 is labelled 'AUDIT REUSE' in the CoT scaffold
        assert.ok(prompt.includes('AUDIT REUSE'), 'Expected AUDIT REUSE step in CoT scaffold');
    });
    test('[HYBRID] prompt should include Gold Standard block when no champion', async () => {
        ...
        const prompt = await service.generateAppiumPrompt('/test/project', 'User logs in with valid credentials', mockConfig, mockAnalysis);
        assert.ok(prompt.includes('GOLD STANDARD'), `Expected GOLD STANDARD block when no champion exists. Prompt snippet: ${prompt.substring(0, 800)}`);
    });
    test('[HYBRID] prompt should include architecture anti-pattern block', async () => {
        const prompt = await service.generateAppiumPrompt('/test/project', 'User logs in with valid credentials', mockConfig, mockAnalysis);
        assert.ok(prompt.includes('ANTI-PATTERN'), 'Expected negative example ANTI-PATTERN block in prompt');
    });
});
...
describe('HybridPromptEngine — quality-weighted champion selection', () => {
    ...
    test('[CHAMPION] balanced 8-method file should beat bloated 30-method file', () => {
        ...
        assert.strictEqual(champion.className, 'NewLoginPage', `Expected NewLoginPage (balanced, 8 methods) to beat OldLoginPage (bloated, 30 methods). Got: ${champion.className}`);
    });
    test('[CHAMPION] AST-warned file should lose to clean smaller file', () => {
        ...
        const champion = engine.selectChampion(analysis);
        assert.ok(champion, 'Expected a champion to be selected');
        assert.strictEqual(champion.className, 'CleanPage', `Expected CleanPage (no AST warnings) to beat LazyPage (AST-warned). Got: ${champion.className}`);
    });
    test('[CHAMPION] base/util classes should be excluded from champion selection', () => {
        ...
        const champion = engine.selectChampion(analysis);
        assert.ok(champion, 'Expected a champion to be selected');
        assert.notStrictEqual(champion.className, 'BasePage', 'BasePage (infrastructure) should never be selected as champion');
        assert.strictEqual(champion.className, 'LoginPage', `Expected LoginPage (real PO) to be selected. Got: ${champion.className}`);
    });
});
