...
describe('TestGenerationService — Issue #11 Fix Validation', () => {
    ...
    test('should generate a valid prompt string', () => {
        const prompt = service.generateAppiumPrompt('/test/project', 'User logs in with valid credentials', mockConfig, mockAnalysis);
    ...
    test('should handle optional parameters (testName, screenXml, screenshotBase64)', () => {
        const prompt = service.generateAppiumPrompt('/test/project', 'User logs in with valid credentials', mockConfig, mockAnalysis, 'LoginTest', undefined, '<xml>test</xml>', 'base64data');
    ...
    test('should include existing code reuse sections', () => {
        ...
        const prompt = service.generateAppiumPrompt('/test/project', 'Test a feature', mockConfig, analysisWithCode);
    ...
    test('should include learning prompt if provided', () => {
        ...
        const prompt = service.generateAppiumPrompt('/test/project', 'User logs in', mockConfig, mockAnalysis, undefined, learningPrompt);
        assert.ok(prompt.includes(learningPrompt));
    });
    test('should specify correct JSON output format with jsonPageObjects', () => {
        const prompt = service.generateAppiumPrompt('/test/project', 'User logs in', mockConfig, mockAnalysis);
