...
describe('CredentialService - manage_users (Issue #14 Fix)', () => {
    ...
    it('should use default src/test-data directory when no config exists', async () => {
        ...
        const result = await credentialService.manageUsers(testProjectRoot, 'write', 'staging', [{ username: 'admin@test.com', password: 'secret123', role: 'admin' }]);
    ...
    it('should respect testDataRoot from mcp-config.json', async () => {
        ...
        fs.writeFileSync(path.join(testProjectRoot, 'mcp-config.json'), JSON.stringify(config, null, 2));
        const result = await credentialService.manageUsers(testProjectRoot, 'write', 'prod', [
    ...
    it('should read users from correct directory based on config', async () => {
        ...
        fs.writeFileSync(path.join(testProjectRoot, 'mcp-config.json'), JSON.stringify(config, null, 2));
        // Write users first
        await credentialService.manageUsers(testProjectRoot, 'write', 'staging', [{ username: 'test@example.com', password: 'testpass', role: 'tester' }]);
        // Read them back
        const result = await credentialService.manageUsers(testProjectRoot, 'read', 'staging');
    ...
    it('should generate getUser helper with correct relative path', async () => {
        ...
        fs.writeFileSync(path.join(testProjectRoot, 'mcp-config.json'), JSON.stringify(config, null, 2));
        await credentialService.manageUsers(testProjectRoot, 'write', 'staging', [{ username: 'helper@test.com', password: 'helperpass' }]);
    ...
    it('should NOT create phantom test-data/ directory at project root', async () => {
        ...
        fs.writeFileSync(path.join(testProjectRoot, 'mcp-config.json'), JSON.stringify(config, null, 2));
        await credentialService.manageUsers(testProjectRoot, 'write', 'staging', [{ username: 'test@example.com', password: 'test123' }]);
    ...
    it('should handle multiple environments correctly', async () => {
        ...
        fs.writeFileSync(path.join(testProjectRoot, 'mcp-config.json'), JSON.stringify(config, null, 2));
        // Write staging users
        await credentialService.manageUsers(testProjectRoot, 'write', 'staging', [{ username: 'staging@test.com', password: 'staging123', role: 'admin' }]);
        // Write prod users
        await credentialService.manageUsers(testProjectRoot, 'write', 'prod', [{ username: 'prod@test.com', password: 'prod456', role: 'readonly' }]);
    ...
    it('should return error message when reading non-existent users file', async () => {
        const result = await credentialService.manageUsers(testProjectRoot, 'read', 'nonexistent');
