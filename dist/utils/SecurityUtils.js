...
export function validateProjectRoot(projectRoot) {
    ...
    if (!safePathRegex.test(projectRoot)) {
        throw new Error(`Invalid projectRoot path: "${projectRoot}". ` +
 ...
 */
export function validateFilePath(projectRoot, filePath) {
    ...
    if (!resolvedFilePath.startsWith(resolvedProjectRoot + path.sep) &&
        ...
        throw new Error(`Path traversal detected: File path "${filePath}" resolves to "${resolvedFilePath}" ` +
 ...
 */
export function auditGeneratedCode(code, filePath) {
    const warnings = [];
 ...
 */
export function auditFeatureFile(content, filePath) {
    const warnings = [];
