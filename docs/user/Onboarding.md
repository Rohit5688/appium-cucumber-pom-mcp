...
### Step 0: Verification (The Scout)
> "Check my project readiness at `/your/project/path` to ensure I have all mobile dependencies (Android SDK, Appium, drivers) installed and the environment is compatible."
...
### Step 3: Architecture & Rule Zeros
> "Analyze my `src/pages/BasePage.ts` and create a summary of accessible methods. I want all future tests to strictly follow the POM pattern. Also, scan for any `// @mcp-learn` comments in my codebase to identify team-specific automation rules we've already established."
...
| **`credentials.strategy`** | Auth strategy & environment? | `"per-env-files"`, `"role-env-matrix"`, or `"custom"`. |
| **`environments`** | Target network pipelines? | `["local", "staging", "prod"]` |
| **`codegen.tagTaxonomy`**| Valid test tags? | `["@smoke", "@regression", "@p0"]` |
| **`timeouts`** | Global & Element timeouts? | `{ "elementWait": 10000, "scenarioTimeout": 60000 }` |
...
Instead of telling the AI to *"Swipe up using W3C actions"*, simply say:
> *"Use the `MobileGestures` utility to scroll until the 'Sign Out' button is visible."*

This ensures the generated code is clean, reusable, and handles coordinate math automatically.
