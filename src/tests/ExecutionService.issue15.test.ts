...
describe('ExecutionService - Issue #15: Valid Locator Strategies', () => {
  ...
  it('should generate valid accessibility-id selector with ~ prefix', async () => {
    ...
    assert.ok(element.locatorStrategies.includes('~Submit Button'), 
      'Should include accessibility-id selector');
    
    // Should NOT contain invalid *[text()="..."] format
    const hasInvalidFormat = element.locatorStrategies.some(s => /^\*\[text\(\)=/.test(s));
    assert.strictEqual(hasInvalidFormat, false, 
      'Should NOT contain invalid *[text()="..."] format');
  });
  ...
  it('should generate valid resource-id selector with id= prefix', async () => {
    ...
    assert.ok(element.locatorStrategies.includes('id=com.app:id/submitBtn'),
      'Should include resource-id selector');
    
    // Should NOT contain invalid selector format
    const hasInvalidFormat = element.locatorStrategies.some(s => /^\*\[/.test(s));
    assert.strictEqual(hasInvalidFormat, false,
      'Should NOT contain selectors starting with *[');
  });
  ...
  it('should generate valid XPath selector for text attribute', async () => {
    ...
    assert.ok(element.locatorStrategies.includes('//*[@text="Login"]'),
      'Should include valid XPath with @text attribute');
    
    // Should NOT generate invalid *[text()="Login"]
    assert.ok(!element.locatorStrategies.includes('*[text()="Login"]'),
      'Should NOT include invalid *[text()= format');
    assert.ok(element.locatorStrategies.every(s => !s.startsWith('*[')),
      'No selector should start with *[');
  });
  ...
  it('should escape double quotes in text for valid XPath', async () => {
    ...
    assert.ok(element.locatorStrategies.includes('//*[@text="He said &quot;Hello&quot;"]'),
      'Should escape double quotes in XPath');
  });
  ...
  it('should prioritize selectors in correct stability order', async () => {
    ...
    assert.strictEqual(strategies[0], '~Submit',
      'Accessibility ID should be first priority');
    ...
    if (resourceIdIndex !== -1 && textXPathIndex !== -1) {
      assert.ok(resourceIdIndex < textXPathIndex,
        'Resource ID should come before text-based XPath');
    }
  });
  ...
  it('should not generate selectors for invalid text (too long)', async () => {
    ...
    assert.ok(element.locatorStrategies.includes('id=com.app:id/longText'),
      'Should include resource-id');
    assert.ok(element.locatorStrategies.every(s => !s.includes('@text=')),
      'Should NOT include text-based XPath for long text');
  });
  ...
  it('should handle iOS elements with name attribute', async () => {
    ...
    assert.ok(element.locatorStrategies.includes('~loginButton'),
      'Should use name attribute for accessibility-id on iOS');
  });
  ...
  it('should generate class-based XPath as last resort', async () => {
    ...
    assert.ok(element.locatorStrategies.includes('//android.widget.Button'),
      'Should use class-based XPath as last resort');
    
    // But this should be the only option
    assert.strictEqual(element.locatorStrategies.length, 1,
      'Should only have class-based selector when no better options');
  });
  ...
  it('regression test: should never generate invalid *[attribute] selectors', async () => {
    ...
    const hasInvalidSelector = element.locatorStrategies.some(s => 
      s.startsWith('*[') || /^\*\[text\(\)=/.test(s)
    );
    
    assert.strictEqual(hasInvalidSelector, false,
      'CRITICAL: Should NOT generate invalid *[text()= selectors');
    
    // Should contain VALID XPath instead
    assert.ok(element.locatorStrategies.includes('//*[@text="Submit"]'),
      'Should generate valid XPath //*[@text="Submit"] instead');
  });
  ...
  it('should work with driver.$() in WebdriverIO', async () => {
    ...
    element.locatorStrategies.forEach(strategy => {
      ...
      assert.ok(matchesValidFormat, 
        `Selector "${strategy}" should match a valid WebdriverIO format`);
    });
  });
});