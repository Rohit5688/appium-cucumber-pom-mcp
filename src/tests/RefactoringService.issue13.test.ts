...
describe('RefactoringService - Custom Directory Layout (Issue #13)', () => {
  ...
  test('[ISSUE #13] should use custom stepDefinitions path from mcp-config.json', async () => {
    ...
    fs.writeFileSync(
      path.join(testProjectDir, 'mcp-config.json'),
...
`;
    ...
    fs.writeFileSync(
      path.join(customStepsDir, 'login.steps.ts'),
      stepContent
    );
    ...
    assert.ok(
      analysis.existingStepDefinitions[0].file.includes('tests/steps'),
      `Step file path should include custom directory, got: ${analysis.existingStepDefinitions[0].file}`
    );
  });
...
`;
    ...
    fs.writeFileSync(
      path.join(customPagesDir, 'LoginPage.ts'),
      pageContent
    );
    ...
    assert.ok(
      analysis.existingPageObjects[0].path.includes('automation/page-objects'),
      `Page object path should include custom directory, got: ${analysis.existingPageObjects[0].path}`
    );
  });
...
Given('user checks out', async () => {});
    ...
    fs.writeFileSync(
      path.join(deepPagesDir, 'CheckoutPage.ts'),
      `
export class CheckoutPage {
  ...
  async submit() { await this.submitButton.click(); }
    ...
    assert.ok(
      analysis.existingStepDefinitions.some(s => s.file.includes('src/test/bdd/step-definitions')),
      'Should find steps in deeply nested directory'
    );
    assert.ok(
      analysis.existingPageObjects.some(p => p.path.includes('src/test/bdd/page-objects')),
      'Should find pages in deeply nested directory'
    );
  });
...
Given('the app is launched', async () => {});  // Duplicate!
    ...
    fs.writeFileSync(
      path.join(customPagesDir, 'HomePage.ts'),
      `
...
When('user goes to settings', async function() {
  await homePage.navigateToSettings();
    ...
    assert.ok(
      report.includes('navigateToProfile') || report.includes('Potentially Unused'),
      'Should detect unused page method'
    );
  ...
  test('[ISSUE #13] integration: full suggest_refactorings workflow with custom paths', async () => {
    ...
    // Create test content
    fs.writeFileSync(
      path.join(customLayout.pages, 'CartPage.ts'),
      `
...
When('user proceeds to checkout', async function() {
  await cartPage.checkout();
    ...
    assert.ok(
      report.includes('clearCart') || report.includes('applyCoupon'),
      'Detected unused methods'
    );
  });
