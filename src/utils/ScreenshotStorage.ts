...
export class ScreenshotStorage {
   ...
   */
  public store(base64Data: string, prefix: string = 'screenshot'): {
    filePath: string;
    relativePath: string;
    timestamp: string;
    size: number;
  ...
  static storeBase64(
    ...
    prefix: string,
    base64Data: string,
  ): { filePath: string; relativePath: string; timestamp: string; size: number } {
   ...
   */
  static scaleDownPng(input: Buffer): Buffer {
    try {
      ...
      if (channels === 0 || bitDepth !== 8) return input;
  ...
  private static _pngChunk(type: string, data: Buffer): Buffer {
    const crc32 = require('zlib') as typeof import('zlib');
  ...
  private static _crc32(buf: Buffer): number {
    let crc = 0xffffffff;
   ...
   */
  public cleanup(maxAgeMs: number = 86400000): {
    removed: number;
    freedBytes: number;
  } {
    let removed = 0;
